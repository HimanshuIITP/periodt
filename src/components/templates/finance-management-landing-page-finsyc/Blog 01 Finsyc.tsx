...
const getTagStyles = (tag: string): string => {
  ...
  if (t === "ai") return "bg-[#E1F3FE] text-[#0369A0]";
  if (t === "innovation") return "bg-[#DCFDE7] text-[#15803C]";
  if (t === "tech") return "bg-[#FFF2C6] text-[#B45309]";
  return "bg-[#F4FBFB] text-[#042619]/60";
};
...
function BlogCard({ image, date, title, description, tags, imageTop = true, delay = 0 }: BlogCardProps) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group cursor-pointer flex flex-col items-start bg-white hover:bg-[#F7FDFE] rounded-[24px] overflow-hidden border border-[#042619]/10 shadow-[0_4px_24px_rgba(4,39,24,0.02)] hover:shadow-[0_20px_60px_rgba(4,39,24,0.08)] transition-all duration-500 w-full lg:w-[612px]"
    >
      {imageTop && (
          ...
          <div className="w-full max-w-[1248px] mx-auto">
            ...
            <div className="flex flex-col items-center text-center mb-12 md:mb-[80px]">
              <motion.div
                ...
                viewport={{ once: true }}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#E4F2EB] border border-[#138E5F]/10 mb-6"
              >
